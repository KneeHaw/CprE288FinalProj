...
void handle_input(char input, oi_t* sensor_data) {
    new_data_flag = 0x00;
    if (input == 'w') {
        move_distance(sensor_data, 500, 50, 0);
    }
    else if (input == 's') {
        ...
        case 'g':
            turn_deg(90, 100);
            turn_deg(saved_obj.smlst_width_ang, 100); //Turn to the smallest object's middle
