...
objs_t quick_dynamic_scan(objs_t prev_objs) {
    ...
    for(i = 52; i <= 136; i+=2){
        turn_servo_to_deg(i);
        timer_waitMillis(6);
        repeated_ir_scan(&sensor_data, 5);

        sprintf(text, "%-20d %-20d %-10d\n", i, sensor_data.IR_dist);
        print_string(text);
        timer_waitMillis(10);
        if(abs(currentDist - sensor_data.IR_dist) < 20) { }
            ...
            if ((i - startAngle > 4) && (currentDist < 50) && (currentDist != -100) && (sensor_data.IR_dist > currentDist)){
                ...
                current_objs.width = current_objs.distance[objects.num_objs] * (current_objs.tot_ang[objects.num_objs] * PI / 180.0)
                ...
                if (num_scans > 1){
                    ...
                    previous_obj_distance = prev_objs.distance[current_objs.num_objs];
                    ...
                    if(currentDist < previous_obj_distance && i > previous_obj_angle) {
                        ...
                        current_objs.civilian_leftwards[current_objs.num_objs] = true;
                        oi_setWheels(0, 0);
                   ...
                   // Should change to if ((current distance > previous_obj_distance * .9) && (current distance < previous_obj_distance * 1.1))?
                       ...
                       current_objs.civilian_rightwards[current_objs.num_objs] = true;
                       oi_setWheels(0, 0);
                       // add code to determine how long to wait
                   }
